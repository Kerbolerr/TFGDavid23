...
df = pandas.read_excel("CleanData/Desktop.ods", engine="odf")


brands = {}
identifier = 0
for brand in df['Marca'].values:
...
for key,value in brands.items():
    modified = [(''.join(str(brand).split(' ')[0]).lower()) for brand in df['Marca'].values]
...
df = df.drop(columns=['Correcto','Muy bueno','Excelente','Marca del procesador','Procesador','Marca','Tarjeta gráfica'])

df.to_excel("CleanData/DesktopTest.ods", index=False)
